 ...
 */
// Deployed Goerli 7-6-23: 0x0b3A9710005a82C35446eAE8B8F8ba5A77988541
...
interface NFTMarketplace {
    ...
    /// @notice Maps all internally held tokens and ETH
    /// @dev Bidder's Address => ERC20 Contract => Releasable Balance
    function balanceOf(address user, IERC20 paymentContract) external view returns(uint256 balance);

    /// @notice Returns the last-created itemID
    function idCounter() external view returns(uint256 itemID);

    /// @notice Stores all Listing structs added to the system, shares itemIDs with Auctions
    /// @dev Auction IDs will yield a default Listing struct
    function allListings(uint256 itemID) external view returns(Listing memory listingData);

    /// @notice Stores all Auction structs added to the system, shares itemIDs with Listings
    /// @dev Listing IDs will yield a default Auction struct
    function allAuctions(uint256 itemID) external view returns(Auction memory auctionData);

    /// @notice Used to determine if an NFT currently has an Auction or Listing that has not been collected
    function itemActive(IERC721 nftContract, uint256 tokenID) external view returns(bool itemIsActive);
    ...
    /// @return ItemType - DOES_NOT_EXIST, LISTING, AUCTION
    ...
    /// @notice Returns the minimum amount of ETH that must be sent to meet the Auction's increment requirement
    function  getMinETHBidAmount(uint256 auctionID) external view returns(uint256 ethAmount);

    /// @notice Bids with a flat amount of ETH, must be at least the minimum bid increment
    function ethBidAmount(uint256 auctionID) external payable;

    /// @notice Bids with a flat amount of ERC20, must be at least the minimum bid increment
    function erc20BidAmount(uint256 auctionID, uint256 bidAmount) external;

    /// @notice Bids with the minimum bid increment, must have sufficient allowance for increment + current bid
    function erc20BidIncrement(uint256 auctionID, uint256 bidIncrement) external;
    ...
    /// @notice Purchases an NFT that was listed for ETH
    function buyETHNFT(uint256 itemID) external payable;

    /// @notice Purchases an NFT that was listed for ERC20
    function buyERC20NFT(uint256 itemID) external;
