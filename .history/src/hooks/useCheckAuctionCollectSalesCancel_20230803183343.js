...
export const useCheckAuctionCollectSalesCancel = (setDisplayButton, setIsOpen, setModalText) => {
                ...
                for (const listing of listingData) {
                    ...
                    if (currentOwner === userWalletAddress) {
                        if (
                            ...
                            currentStatus === 3
                            &&
                            (await marketplaceContract.getAuction(listing.listingId)).topBidder === ETHEREUM_NULL_ADDRESS
                        ) {
                            usersExpiredAuctions.push({
                                listingId: listing.listingId,
                                tokenId: tokenId,
                                nftContractAddress: nftContractAddress
                            });
                        ...
                        currentStatus === 4
                        &&
                        (await marketplaceContract.getAuction(listing.listingId)).topBidder === userWalletAddress
                    ) {
                        usersWonAuctions.push({
                            listingId: listing.listingId,
                            tokenId: tokenId,
                            nftContractAddress: nftContractAddress
                        });
