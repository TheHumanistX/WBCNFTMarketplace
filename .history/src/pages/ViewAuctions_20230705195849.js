...
const ViewAuctions = () => {
  ...
  const bidWithETH = async (bidAmount, auctionID) => {
    try {
      ...
      const transaction = await marketplaceContract.call('bid', [ethers.utils.parseEther(bidAmount.toString())], auctionID, { value: ethers.utils.parseEther(bidAmount.toString()) });
      await transaction.wait(); // Wait for the transaction to be mined
      console.log('Bid placed with ETH!');
    } catch (error) {
      console.error('Failed to bid with ETH', error);
