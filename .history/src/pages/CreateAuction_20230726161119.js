...
const CreateAuction = () => {
    ...
    const handleCreateAuction = async (auctionDetails) => {
        if (isNaN(auctionDetails.initialBidAmount)) {
            setIsOpen(true);
            setModalText('Please enter a valid initial bid amount.');
            return;
        }
        if (isNaN(auctionDetails.bidIncrement)) {
            setIsOpen(true);
        ...
        if (new Date(auctionDetails.auctionBeginTime) < now || new Date(auctionDetails.auctionEndTime) < now) {
            ...
            alert("Both the beginning and ending time for the auction must be in the future.");
            return;
        }
        let approval;
        const date = new Date();
