...
import { useCheckAuctionCollectSalesCancel, useSpendWithWBC } from '../hooks';
...
const BuyNFT = () => {
  ...
  const { spendWithWBC, isOpen, setIsOpen, modalText } = useSpendWithWBC({ setIsOpen, setModalText });
  const { spendWithETH, isOpen, setIsOpen, modalText } = useSpendWithETH();
  ...
  const buyWithWBC = async (listingPrice, listingID) => {
    await spendWithWBC(listingPrice, listingID, setTxConfirm, path);
  }
