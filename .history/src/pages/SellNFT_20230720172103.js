  ...
  const handleListNFTForSale = async (e) => {
    ...
    if (saleCurrency === 'ETH') {
      ...
      if (approval && approval.status === 1) {
        try {
          console.log('Try statement marketplaceContract: ', marketplaceContract)
      ...
      if (approval && approval.status === 1) {
        try {
          const transactionResponse = await marketplaceContract.createListing(nftContractAddress, tokenContractAddress, tokenId, priceInWei);
          const transactionReceipt = await transactionResponse.wait();
          if (transactionReceipt.status === 1) {
            console.info("Contract call successs", transactionReceipt);
            setTxConfirm(transactionReceipt.blockHash);
