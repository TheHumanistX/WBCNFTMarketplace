...
const ViewAuctions = () => {
  ...
  const bidWithWBC = async (bidAmount, auctionID, minimumAllowableBid, auctionExpiration) => {
    ...
    console.log('STEP 1: Entered bidWithWBC in ViewAuctions...')
    try {
  ...
  // ! Below this is old version for bidding... trying to set this up (above) to work like BuyNFT
  ...
  const bidWithETH = async (bidAmount, auctionID, minimumAllowableBid, auctionExpiration) => {
    if (!await bidOnAuctionInputChecks(bidAmount, minimumAllowableBid, setIsOpen, setModalText)) return;
    ...
    <section className='buynft__container'>
      ...
      <div className='buy__owned-flex'>
        {liveAuctions && liveAuctions.map((auction, index) => {
          return (
            <div key={index}>
              <ShowListedNFTs listing={auction} marketplaceContract={marketplaceContract} bidWithETH={bidWithETH} bidWithWBC={bidWithWBC} setTimerComplete={setTimerComplete} />
            </div>
          )
        }
        )}
