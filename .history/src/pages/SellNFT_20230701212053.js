...
const SellNFT = () => {
  ...
  useEffect(() => {
    const fetchNFTs = async () => {
      const lastTokenId = await nftContract.call("getLastTokenID", []);
      ...
      for (let i = 1; i <= lastTokenId; i++) {
        ...
        if (owner.toLowerCase() === userWalletAddress.toLowerCase()) {
          ...
          for (let j = 1; j <= lastListingId; j++) {
            const listing = await marketplaceContract.call("getListing", [j]);
  
            if (listing.tokenID === i && listing.status === "ACTIVE") {
              isListed = true;
              break;
            }
          }
