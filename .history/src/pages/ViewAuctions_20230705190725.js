...
const ViewAuctions = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      const currentListings = marketplaceAuctionCreatedEvents.map((event) => ({
        ...
        minBindings: event.data.minBidIncrement,
  ...
  const bidWithWBC = async (bidAmount, auctionID) => {
    ...
    if (approval && approval.receipt.status === 1) {
      try {
        // console.log('Buying...')
        const transaction = await buyNFT({ args: [auctionID, bidAmount] });
  ...
  const bidWithETH = async (bidAmount, auctionID) => {
    try {
      ...
      const transaction = await marketplaceContract.call('bid', [auctionID, ethers.utils.parseEther(bidAmount.toString())], { value: bidAmount });
      await transaction.wait(); // Wait for the transaction to be mined
      console.log('Bid placed with ETH!');
    } catch (error) {
      console.error('Failed to bid with ETH', error);
