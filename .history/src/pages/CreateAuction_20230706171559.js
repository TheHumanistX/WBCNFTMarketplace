...
const CreateAuction = () => {
    ...
    const [refreshOwnedNFTs, setRefreshOwnedNFTs] = useState(false);
    ...
    const ownedNFTs = useOwnedNFTs(lastNFTMintedId, nftContract, marketplaceContract, userWalletAddress, refreshOwnedNFTs);
        ...
        if (saleCurrency == 'ETH') {
            ...
            if (approval && approval.receipt.status === 1) {
                try {
                    ...
                    if (data && data.receipt && data.receipt.status === 1) {
                        console.log('Auction created, refreshing NFTs list...');
                        setRefreshOwnedNFTs(prev => !prev);
                    }
                } catch (err) {
                    console.error("contract call failure", err);
                }
            ...
            if (approval && approval.receipt.status === 1) {
                try {
                    const data = await createAuction({ args: [nftContractAddress, tokenId, tokenAddress, initBidInWei, bidIncrementInWei, beginTimestamp, expirationTimestamp] });
                    console.info("contract call successs", data);
                    if (data && data.receipt && data.receipt.status === 1) {
                        console.log('Auction created, refreshing NFTs list...');
                        setRefreshOwnedNFTs(prev => !prev);
