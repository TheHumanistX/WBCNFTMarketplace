...
const SellNFT = () => {
  ...
  const ownedNFTs = useOwnedNFTs( nftContract, marketplaceContract, userWalletAddress, lastNFTMintedId, txConfirm);
  ...
  const handleListNFTForSale = async (e) => {
    ...
    if (saleCurrency === 'ETH') {
      ...
      if (approval && approval.receipt.status === 1) {
        try {
          const transaction = await createListing({ args: [nftContractAddress, tokenId, priceInWei] });
          console.log('transaction: ', transaction)
          if (transaction.status === 1) {
          console.info("contract call successs", transaction  );
          setTxConfirm(transaction.hash);
          } else {
      ...
      if (approval && approval.receipt.status === 1) {
        try {
          const transaction = await createListing({ args: [nftContractAddress, tokenAddress, tokenId, priceInWei] });
          if (transaction.status === 1) {
          console.info("contract call successs", transaction);
          setTxConfirm(transaction.hash);
          } else {
