...
const ViewAuctions = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      const auctionDetails = await Promise.all(
        ...
        console.log('auctionDetails[0]: ', auctionDetails[0][8])
      const listingStatuses = auctionDetails.map(auctionDetail => auctionDetail[8]);  // Assuming status is at index 6 of the returned tuple
      const minBidIncrements = auctionDetails.map(auctionDetail => auctionDetail[5].toNumber());  // Assuming minBidIncrement is at index 4 of the returned tuple
      const currentBids = auctionDetails.map(auctionDetail => auctionDetail[4].toNumber());  // Assuming currentBid is at index 4 of the returned tuple
  ...
  const bidWithETH = async (bidAmount, auctionID) => {
    try {
      ...
      const weiBidAmount = ethers.utils.parseEther(bidAmount.toString(), 'wei');
      // console.log('bidWithETH weiBidAmount: ', weiBidAmount)
      // console.log('bidWithETH toNUmber: ', weiBidAmount.toNumber() )
      const transaction = await marketplaceContract.call('bid', [weiBidAmount, auctionID], { value: weiBidAmount });
      await transaction.wait(); // Wait for the transaction to be mined
      console.log('Bid placed with ETH!');
    } catch (error) {
      console.error('Failed to bid with ETH', error);
