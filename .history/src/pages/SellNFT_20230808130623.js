...
import { approveNFTTransfer, createNewListing, createNewSaleListingChecks } from '../utility';
...
const SellNFT = () => {
  ...
  const handleListNFTForSale = async (listingDetails) => {
    if (!await createNewSaleListingChecks(listingDetails, setIsOpen, setModalText)) return;
    ...
    try {
      ...
      setIsOpen(true);
      setModalText('Error during listing creation. See console for further information.')
