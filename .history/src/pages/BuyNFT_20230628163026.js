...
import { CheckNFTStatus, ShowListedNFTs } from '../components'
...
const BuyNFT = () => {
  ...
  const handleStatusCheckCompleted = (listingID, status) => {
    setListingStatuses(prevStatuses => ({ ...prevStatuses, [listingID]: status }));
  };
    ...
    <section className='buynft__container'>
      ...
      <div className='buynft__owned-grid'>
        ...
        {liveListings && liveListings.map((listing, index) => {
          return (
            <div key={index}>
              {console.log('Entering `CheckNFTStatus component...')}
              <CheckNFTStatus listingID={listing.listingID} onCheckCompleted={(status) => handleStatusCheckCompleted(listing.listingID, status)} />
              {listingStatuses[listing.listingID] &&
              <>
