...
const SellNFT = () => {
  ...
  const handleListNFTForSale = async (e) => {
    ...
    if (listingCurrency === 'ETH') {
      ...
      try {
        await listNFT({ setTxConfirm, marketplaceContract, nftContractAddress, tokenId, priceInWei, listingCurrency });
      } catch (err) {
        console.error("contract call failure", err);
      }
    } else {
      approval = await handleApprove(marketplaceContractAddress, tokenId);
      try {
        await listNFT({ setTxConfirm, marketplaceContract, nftContractAddress, tokenContractAddress, tokenId, priceInWei, listingCurrency });
      } catch (err) {
        console.error("contract call failure", err);
      }
    }
