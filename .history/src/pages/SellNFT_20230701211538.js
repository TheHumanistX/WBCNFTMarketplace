...
const SellNFT = () => {
  ...
  useEffect(() => {
    ...
    console.log('Entering useEffect in SellNFT.js... userWalletAddress & nftContract passed...')
    ...
    const fetchNFTs = async () => {
        ...
        for (let i = 1; i <= lastNFTMintedId; i++) {
            console.log('For loop running... ', i, ' of ', lastNFTMintedId )
            
            const owner = await nftContract.call("ownerOf", [i]);
            if (owner.toLowerCase() === userWalletAddress.toLowerCase()) {
                const listing = await marketplaceContract.call("getListing", [marketplaceNFTListedEvents.length - 1]);
