...
import { approveTokenSpend, tokenSpend, ethSpend } from '../utility';
...
const ViewAuctions = () => {
  ...
  const bidWithWBC = useCallback(async (bidAmount, auctionID, formattedMinBidIncrement) => {
    if(!await bidOnAuctionInputChecks(bidAmount, formattedMinBidIncrement, setIsOpen, setModalText)) return;
  ...
  const bidWithETH = useCallback(async (bidAmount, auctionID, formattedMinBidIncrement) => {
    ...
    const minETHBidAmount = await marketplaceContract.getMinETHBidAmount(auctionID)
    ...
    if (parseFloat(bidAmount) < parseFloat(formattedMinBidIncrement + bidAmount)) {
    ...
    try {
      ...
      console.error('Failed to bid with ETH', err);
    }
