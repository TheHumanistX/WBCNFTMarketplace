...
const BuyNFT = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const currentListings = parsedLogs ? parsedLogs.map((log) => log.args.to) : '';
  ...
  return (
    <section className='buynft__container'>
      <h1>Buy An NFT!</h1>
      <h3>There are {recentTransactions.length} NFTs listed for sale!</h3>
      <ShowListedNFTs liveListings={liveListings} />
      {console.log('liveListings', liveListings)}
    </section>
  )
}
