  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (result.distance <= 0) {
        console.log('Entered if statement in useEffect in TimerRemaining because result.distance <= 0')
        setTimerComplete(!timerComplete);
        clearInterval(timer);
      } else if (expiration * 1000 < Date.now()) {
        console.log('Entered ELSEif statement in useEffect in TimerRemaining because result.distance <= 0')
        setTimerComplete(!timerComplete);
        clearInterval(timer);
      }
    }, 1000);
