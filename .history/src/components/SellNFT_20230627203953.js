...
const SellNFT = () => {
  ...
  const { data: transferEvents } = useContractEvents(contract, "Transfer", {
    queryFilter: {
      order: "asc",
    },
    subscribe: true
  ...
  console.log('listingCreated: ', listedNFTEvents ? listedNFTEvents : '');
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      let tokenIdsOwnedByWallet = Object.values(transferEvents)
        .filter(event => event.data.to.toLowerCase() === walletAddress.toLowerCase())
        .map(event => event.data.tokenId.toNumber());
      let tokenIdsForCreatedListings = Object.values(listedNFTEvents)
        .filter(event => event.data.owner.toLowerCase() === walletAddress.toLowerCase())
        .map(event => event.data.tokenId.toNumber());
    ...
    fetchEvents();
  }, [contract, transferEvents, listedNFTEvents]);
