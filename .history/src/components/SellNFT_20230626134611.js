...
const SellNFT = () => {
  ...
  console.log('contractName', contractName ? contractName : '');
  console.log('ownedNFTs', ownedNFTs ? ownedNFTs : '0')
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      console.log('parsedLogs', JSON.stringify(parsedLogs.logDescription.args));
      ...
      for (let i = 0; i < transactions.length; i++) {
        ...
        if (element.to === walletAddress) {
          ...
          ownerTokenIds.push(element.tokenId)
        }
      }
      setOwnedNFTs(ownerTokenIds);
  ...
  return (
    <section className='sellNFT__container'>
      ...
      <span>Tokens of currently held nfts are:</span>
