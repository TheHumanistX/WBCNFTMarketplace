...
const BuyNFT = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const parsedLogs = logs.map((log) => contract.interface.parseLog(log.args));
      console.log('parsed logs: ', parsedLogs ? parsedLogs : '');
      setLiveListings(parsedLogs);
    ...
    <section className='buynft__container'>
      ...
      {liveListings && liveListings.map((listing, index) => {
            return (
                ...
                <ShowListedNFTs nftId={listing.tokenID} marketplaceContract={marketplaceContract} />
