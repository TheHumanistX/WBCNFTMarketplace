...
const BidWithETH = ({
  ...
  <div>
    ...
    <button className='bidOnNFT__button' onClick={() =>
      handleBidWithETH(bidAmount, auctionID, minimumAllowableBid, auctionExpiration)
    }
    >
      Bid with ETH
    </button>
...
const BidWithWBC = ({
  ...
  <div>
    ...
    <button className='bidOnNFT__button' onClick={() =>
      handleBidWithWBC(bidAmount, auctionID, minimumAllowableBid, auctionExpiration)
    }
    >
      Bid with {tokenSymbol}
    </button><br />
    <button className='bidOnNFTIncrement__button' onClick={() =>
      bidWithWBC(`${minBidAmount}`, auctionID, auctionExpiration)
    }>
      Bid Min Increment
    </button>
  </div>
...
const BidOnNFT = ({ bidWithETH, bidWithWBC, auctionID, minBidIncrement, formattedCurrentBid, formattedMinBidIncrement, tokenSymbol, paymentContractAddress, auctionExpiration }) => {
  ...
  const handleBidWithETH = (bidAmount, auctionID, minimumAllowableBid, auctionExpiration) => {
    bidWithETH(bidAmount, auctionID, minimumAllowableBid, auctionExpiration);
    setBidAmount('')
  }

  const handleBidWithWBC = (bidAmount, auctionID, minimumAllowableBid, auctionExpiration) => {
    bidWithWBC(bidAmount, auctionID, minimumAllowableBid, auctionExpiration)
    setBidAmount('')
  }
