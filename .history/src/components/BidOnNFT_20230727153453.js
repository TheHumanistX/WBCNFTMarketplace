...
const BidOnNFT = ({ bidWithETH, bidWithWBC, auctionID, minBidIncrement, formattedCurrentBid, formattedMinBidIncrement, tokenSymbol, paymentContractAddress }) => {
  ...
  const handleBidWithETH = (bidAmount, auctionID) => {
    ...
    bidWithETH(bidAmount, auctionID, formattedMinBidIncrement)

    setBidAmount('')
  
  }
  const handleBidWithWBC = (bidAmount, auctionID, formattedMinBidIncrement) => {
    
    bidWithWBC(bidAmount, auctionID, formattedMinBidIncrement)
      ...
      {paymentContractAddress === ETHEREUM_NULL_ADDRESS ?
        <div>
          ...
          <button className='bidOnNFT__button' onClick={() =>
              handleBidWithETH(bidAmount, auctionID, formattedMinBidIncrement)
          }
          >
            Bid with ETH
          </button>
        ...
        <div>
          ...
          <button className='bidOnNFT__button' onClick={() =>
              handleBidWithWBC(bidAmount, auctionID, formattedMinBidIncrement)
          }
          >
