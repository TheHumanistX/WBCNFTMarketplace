...
const BuyNFT = () => {
  ...
  const { data: name } = useContractRead(WBCContract, 'name');
  ...
  const buyWithWBC = async (listingPrice, listingID) => {
    let approval;
    try {
      approval = await approve({ args: [marketplaceContractAddress, ethers.utils.parseEther(listingPrice)] });
  ...
  const buyWithETH = async (listingPrice, listingID) => {
    try {
      ...
      // Assuming your marketplaceContract has a buyNFT function and it accepts ETH
      const transaction = await buyNFT({ args: [listingID] });
    ...
    <section className='buynft__container'>
      ...
      {liveListings && liveListings.map((listing, index) => {
            return (
                ...
                <button className='buynft__button' onClick={listing.tokenContractAddress === '0x0000000000000000000000000000000000000000' ? () => buyWithETH(listing.price, listing.listingID) : buyWithWBC(listing.price, listing.listingID)}>Buy with 
