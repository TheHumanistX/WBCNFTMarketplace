...
const ShowListedNFTs = ({ listing, marketplaceContract, buyWithETH, buyWithWBC }) => {
    ...
    console.log('currentListing.formattedPrice: ', currentListing.formattedPrice)
    return (
        <div className='listed-nft__flex'>
            ...
            {path === '/view_auctions' && nftData &&
                <>
                    <BidOnNFTPayment buyWithETH={buyWithETH} buyWithWBC={buyWithWBC} listingID={currentListing.listingID} price={currentListing.price} tokenSymbol={tokenSymbol} paymentContractAddress={currentListing.paymentContractAddress} />
                    <span>Auction ID: {currentListing.auctionID + 1}</span>
                    <a href={`https://testnets.opensea.io/assets/goerli/${nftContract}/` + (currentListing.tokenId)}><img src={`https://ipfs.io/ipfs/${nftData.image.split('ipfs://')[1]}`} alt={nftData.name} className='listed-nfts' /></a>
                    <span>{nftContractName} #{currentListingtokenId + 1}</span>
                    ...
                    <span>Initial Bid: {
                        paymentContractAddress === ETHEREUM_NULL_ADDRESS
                            ?
                            currentListing.initialBid + ' ETH'
                            :
                            currentListing.initialBid + ' ' + tokenSymbol
