...
const SellNFT = () => {
  ...
  // const { mutateAsync: createListing, isLoading } = useContractWrite(marketplaceContract, "createListing")
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      const tokenIdsOwnedByWallet = parsedLogs ? parsedLogs.map((log) => {
        ...
        console.log('log.args.to: ', log.args.to)
        if (log.args && log.args.to && log.args.to.toLowerCase() === walletAddress.toLowerCase()) {
