...
const SellNFT = () => {
  ...
  const { mutateAsync: createListing, isLoading } = useContractWrite(marketplaceContractAddress, "createListing")
  ...
  const handleListNFTForSale = async (e) => {
    ...
    if (saleCurrency == 'ETH') {
      ...
      const call = async () => {
        try {
          const data = await createListing({ args: [contractAddress, tokenId, price] });
