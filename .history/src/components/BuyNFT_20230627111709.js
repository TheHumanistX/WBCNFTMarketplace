    ...
    <section className='buynft__container'>
      ...
      {liveListings && liveListings.map((listing, index) => {
            return (
                ...
                <button className='buynft__button' onClick={listing.tokenContractAddress === '0x0000000000000000000000000000000000000000' ? () => buyWithETH(listing.price, listing.listingID) : buyWithWBC(listing.price, listing.listingID)}>Buy with WBC</button>
                </div>
            )
        }
      )}
